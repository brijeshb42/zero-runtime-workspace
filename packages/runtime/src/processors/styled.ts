...
export class StyledProcessor extends BaseProcessor {
  ...
  constructor(params: Params, ...args: TailProcessorParams) {
    ...
    this.componentMetaArg =
      componentMetaArg && componentMetaArg.kind === ValueType.LAZY
        ? componentMetaArg
        : undefined;
    ...
    if (callType === 'member') {
      ...
      switch (componentArg.kind) {
        case ValueType.CONST:
          this.component =
            typeof componentArg.value === 'string'
              ? componentArg.value
              : undefined;
          break;
        case ValueType.LAZY:
  ...
  doRuntimeReplacement(): void {
    ...
    const varProperties: ReturnType<typeof t.objectProperty>[] =
  ...
  processCss(
    ...
    const styleObj =
      typeof styleObjOrFn === 'function'
        ? styleObjOrFn(themeArgs)
        : styleObjOrFn;
    if (!styleObj) {
      return '';
    }
